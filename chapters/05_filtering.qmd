...
Este capítulo documenta el filtrado downstream aplicado a la tabla de ASVs generada en el preprocesamiento. Su función no es volver a procesar FASTQ, sino definir el **set de ASVs que alimenta los análisis biológicos posteriores**. El filtrado parte del objeto importado con todas las muestras y genera dos tipos de salida: un objeto global filtrado, útil para mock/feed/composición, y un subconjunto intestinal para diversidad, abundancia diferencial y análisis funcional.

El run documentado es `run_02_optimized` con taxonomía primaria `silva_138_2`. El filtrado se aplicó a **todas las muestras importadas**: intestino, mock y pienso/feed. Esta decisión mejora la trazabilidad porque los controles técnicos y las muestras de pienso no quedan fuera de la auditoría, aunque los análisis biológicos principales de peces se ejecutan con los objetos intestinales `ps_biological_*`.
...
El filtro taxonómico elimina ASVs asignadas a `Eukaryota` o `Unassigned`, ASVs sin clasificación en todos los rangos disponibles, y ASVs clasificadas únicamente a nivel de reino. En cambio, cloroplasto y mitocondria quedan retenidos. Esta diferencia es clave: se elimina señal taxonómicamente inutilizable para microbiota bacteriana, pero se conserva señal organelar interpretable en el contexto del pienso.

### Filtro de prevalencia

La prevalencia se calcula dentro de grupos definidos por el diseño experimental, no sobre todo el dataset mezclado. Los grupos y umbrales aplicados se resumen en la @tbl-prevalence-groups. Para intestino se usa `diet_time`, para feed se usa el tipo de pienso y para mock se usa el grupo técnico `Mock`.
...
::: {.callout-note title="Por qué el umbral baja a 1 en feed"}
Las muestras de feed tienen una única muestra por dieta. Por eso el umbral absoluto se limita al tamaño real del grupo: si se exigieran 2 muestras, todo el feed quedaría artificialmente eliminado.
:::
