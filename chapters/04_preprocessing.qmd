...
La optimización se reparte en dos herramientas y conviene leerla así: `cutadapt` resuelve la retirada de primers y decide qué pares llegan a `DADA2`; `DADA2` aplica el filtrado por calidad, aprende errores, infiere ASVs, fusiona pares y retira quimeras. Separar ambos pasos evita mezclar dos decisiones técnicas con consecuencias distintas.

## cutadapt: retirada de primers

`cutadapt` [@martin2011cutadapt] se utilizó para retirar los primers de amplificación de las lecturas paired-end antes de entrar en `DADA2`. Esta etapa no infiere ASVs ni corrige errores; su objetivo es producir FASTQ recortados, auditables y listos para el filtrado de calidad posterior.

Los parámetros de `cutadapt` se resumen en la @tbl-cutadapt-params. La decisión clave del run optimizado fue `discard_untrimmed = FALSE`: los pares sin primer detectado no se descartaron automáticamente. Esto evita perder lecturas útiles por fallos de detección del primer, especialmente cuando R2 presenta peor calidad o cuando el primer contiene bases degeneradas.

| Parámetro | Valor | Interpretación |
|------------------------|------------------------|------------------------|
| Forward primer | `CCTAYGGGRBGCASCAG` | Primer forward de la región 16S V3-V4. |
| Reverse primer | `GGACTACNNGGGTATCTAAT` | Primer reverse de la región 16S V3-V4. |
| Error rate | `0.1` | Tolerancia de error para detectar el primer. |
| Minimum overlap | `3` | Solapamiento mínimo requerido entre primer y lectura. |
| Minimum length | `1` | Longitud mínima permitida tras recorte. |
| Match read wildcards | `TRUE` | Permite interpretar bases ambiguas en la lectura durante el matching. |
| Quality trimming | `-q 0,0` | No se aplica recorte por calidad en `cutadapt`; se delega a `DADA2`. |
| `discard_untrimmed` | `FALSE` | Conserva pares sin primer detectado para no descartar señal prematuramente. |

: Parámetros de `cutadapt` en `run_02_optimized` {#tbl-cutadapt-params}

::: {.callout-note title="Qué hace y qué no hace cutadapt"}
`cutadapt` retira secuencias técnicas, pero **no decide por sí solo qué lecturas son biológicamente fiables**. En este pipeline, la criba de calidad principal se delega a `DADA2` mediante `maxEE`, `maxN` y el modelo de errores.
:::

### Control de calidad antes y después del recorte

Los perfiles de calidad crudos y recortados se revisaron para comprobar si la retirada de primers generaba lecturas coherentes y para decidir los parámetros de `DADA2`. La @fig-raw-r1-quality y la @fig-raw-r2-quality muestran los perfiles de calidad de las lecturas crudas R1 y R2. La @fig-trimmed-r1-quality y la @fig-trimmed-r2-quality muestran el estado tras `cutadapt`.

![Perfil de calidad de las lecturas crudas R1 del run `run_02_optimized`.](../assets/results/04_preprocessing/figures/run_02_optimized_raw_R1_quality.svg){#fig-raw-r1-quality fig-align="center" width="90%"}

![Perfil de calidad de las lecturas crudas R2 del run `run_02_optimized`.](../assets/results/04_preprocessing/figures/run_02_optimized_raw_R2_quality.svg){#fig-raw-r2-quality fig-align="center" width="90%"}

![Perfil de calidad de las lecturas R1 tras retirada de primers con `cutadapt`.](../assets/results/04_preprocessing/figures/run_02_optimized_trimmed_R1_quality.svg){#fig-trimmed-r1-quality fig-align="center" width="90%"}

![Perfil de calidad de las lecturas R2 tras retirada de primers con `cutadapt`.](../assets/results/04_preprocessing/figures/run_02_optimized_trimmed_R2_quality.svg){#fig-trimmed-r2-quality fig-align="center" width="90%"}

En `run_02_optimized`, `cutadapt` escribió de nuevo **9,191,390 pares**, es decir, una **retención del 100% de los pares de entrada**. Este valor no debe interpretarse como que el 100% de las lecturas tuviera primer detectado: se obtiene porque `discard_untrimmed = FALSE`, de modo que los pares sin primer identificado se conservaron para que la decisión de calidad principal recayera en `DADA2`. La detección de adapter fue alta en R1 (**mediana 99.65%**) y R2 (**mediana 99.30%**), aunque R2 mostró más variabilidad, por lo que mantener las lecturas y filtrar después por `maxEE` fue una decisión conservadora.

## DADA2: filtrado, ASVs y depuración de errores

`DADA2` [@callahan2016dada2] concentra la parte inferencial del preprocesamiento: filtra lecturas, aprende el modelo de errores, infiere ASVs, fusiona pares y elimina quimeras. Este es el paso que convierte FASTQ recortados en una matriz de ASVs no quiméricas.

### Filtrado de lecturas con `filterAndTrim`

Los parámetros de filtrado de `DADA2` se resumen en la @tbl-dada2-filter-params. El cambio más influyente frente al run inicial fue `maxEE = c(3, 3)`, que relaja moderadamente el número esperado de errores permitidos por lectura. Esta decisión recupera lecturas informativas sin permitir bases ambiguas, porque `maxN = 0` se mantiene estricto.

| Parámetro | Valor | Interpretación |
|------------------------|------------------------|------------------------|
| `truncLen` | `c(0, 0)` | No se aplica truncado fijo por posición. |
| `trimLeft` | `c(0, 0)` | No se recortan bases adicionales al inicio tras `cutadapt`. |
| `maxN` | `0` | No se permiten bases ambiguas. |
| `maxEE` | `c(3, 3)` | Máximo de errores esperados en forward y reverse. |
| `truncQ` | `2` | Truncado terminal si aparece calidad muy baja. |
| `rm.phix` | `TRUE` | Elimina lecturas compatibles con PhiX. |

: Parámetros de filtrado `DADA2::filterAndTrim` {#tbl-dada2-filter-params}
...
![Resumen de calidad Q10 por posición para lecturas crudas y recortadas. El eje X indica la posición en la lectura y el eje Y el percentil 10 de calidad; valores más bajos indican que al menos el 10% de las lecturas presenta calidad reducida en esa posición.](../assets/results/04_preprocessing/figures/run_02_optimized_quality_q10_by_position.svg){#fig-quality-q10-position fig-align="center" width="90%"}

### Modelo de errores

`DADA2` aprendió modelos de error independientes para lecturas forward y reverse. Los modelos resultantes se muestran en la @fig-error-forward y la @fig-error-reverse. En estos gráficos, los puntos representan tasas de error observadas para cada tipo de sustitución nucleotídica y las líneas representan el modelo de error aprendido. Un modelo adecuado muestra una relación decreciente entre calidad y probabilidad de error, con las tasas observadas próximas a las curvas ajustadas.

![Modelo de errores aprendido por `DADA2` para lecturas forward. Los puntos muestran errores observados y las líneas el modelo ajustado para cada transición nucleotídica; la concordancia visual indica que el modelo es compatible con los datos forward.](../assets/results/04_preprocessing/figures/run_02_optimized_errF.svg){#fig-error-forward fig-align="center" width="90%"}

![Modelo de errores aprendido por `DADA2` para lecturas reverse. La mayor dispersión relativa frente a R1 es esperable cuando R2 presenta peor calidad terminal, pero el patrón global mantiene coherencia suficiente para la inferencia de ASVs.](../assets/results/04_preprocessing/figures/run_02_optimized_errR.svg){#fig-error-reverse fig-align="center" width="90%"}

### Inferencia de ASVs, fusión y quimeras

La inferencia de ASVs se ejecutó sin pooling (`pool = FALSE`) y sin self-consistency iterativa (`selfConsist = FALSE`). La fusión de pares fue estricta (`maxMismatch = 0`) porque el éxito de merging ya era alto y permitir mismatches habría reducido confianza más que resolver un cuello de botella real. La retirada de quimeras se realizó con `removeBimeraDenovo(method = "consensus")`.

| Etapa DADA2          | Parámetro     | Valor       |
|----------------------|---------------|-------------|
| `learnErrors`        | `randomize`   | `TRUE`      |
| `learnErrors`        | `nbases`      | `100000000` |
| `dada`               | `pool`        | `FALSE`     |
| `dada`               | `selfConsist` | `FALSE`     |
| `mergePairs`         | `minOverlap`  | `12`        |
| `mergePairs`         | `maxMismatch` | `0`         |
| `removeBimeraDenovo` | `method`      | `consensus` |

: Parámetros de inferencia, fusión y eliminación de quimeras en `DADA2` {#tbl-dada2-denoise-params}
...
La @fig-preprocessing-read-tracking-sample-type muestra la trayectoria de lecturas a través de las etapas principales del preprocesamiento. La caída más clara aparece entre la fusión de pares y la eliminación de quimeras, especialmente en mock y feed, mientras que las muestras intestinales mantienen una retención alta hasta el objeto no quimérico final.

![Trayectoria de lecturas por tipo de muestra durante el preprocesamiento. Las líneas muestran la suma de pares/lecturas en cada etapa: pares crudos, pares escritos tras `cutadapt`, lecturas tras `filterAndTrim`, pares fusionados y lecturas no quiméricas.](../assets/results/04_preprocessing/figures/run_02_optimized_read_tracking_by_sample_type.svg){#fig-preprocessing-read-tracking-sample-type fig-align="center" width="90%"}
...
| Parámetro                 | Valor                                        |
|---------------------------|----------------------------------------------|
| Base primaria             | `SILVA 138.2`                                |
| Training set              | `silva_nr99_v138.2_toSpecies_trainset.fa.gz` |
| Species set               | `silva_v138.2_assignSpecies.fa.gz`           |
| `assignTaxonomy(minBoot)` | `80`                                         |
| `tryRC`                   | `FALSE`                                      |
| `addSpecies`              | `allowMultiple = TRUE`                       |

: Parámetros de asignación taxonómica SILVA {#tbl-taxonomy-params}
...
| `run_02_optimized_read_tracking_by_sample_type.csv` | Datos resumidos usados para la figura de trayectoria de lecturas por tipo de muestra. | <a href="../assets/results/04_preprocessing/tables/run_02_optimized_read_tracking_by_sample_type.csv" download>Descargar CSV</a> |
